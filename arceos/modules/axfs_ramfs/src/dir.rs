...
impl VfsNodeOps for DirNode {
    ...
    fn rename(&self, _src_path: &str, _dst_path: &str) -> VfsResult {
        log::debug!("rename at ramfs: {} to {}", _src_path, _dst_path);
        let src_trimmed = _src_path.trim_end_matches('/');
        let dst_trimmed = _dst_path.trim_end_matches('/');
        let src = src_trimmed.rsplit('/').next().ok_or(VfsError::InvalidInput)?;
        let dst = dst_trimmed.rsplit('/').next().ok_or(VfsError::InvalidInput)?;

        let mut children = self.children.write();
        if let Some(node) = children.remove(src) {
            children.insert(dst.to_string(), node);
            Ok(())
        } else {
